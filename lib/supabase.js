...
// Every reader below used to destructure only `data` and drop `error` on the
// floor. That made a failed read look exactly like an empty table, and the
// page components did the reasonable thing with an empty array: rendered their
// empty state.
//
// On Sept 14, 2026 that produced a live claim tracker reading "No tracked
// claims yet" while 135 claims sat intact in the database, because the
// Supabase REST layer was returning 502 and 504 for about sixteen minutes.
// Nothing threw, so the render counted as a success and ISR cached it.
//
// Throwing is the correct behaviour for a static route: Next abandons the
// regeneration and keeps serving the last good page. Stale beats wrong,
// especially on a site whose entire claim is that its record is complete.
function unwrap(what, { data, error }) {
  if (error) {
    throw new Error(`Supabase read failed (${what}): ${error.message || error}`);
  }
  return data;
}
...
export async function getLatestBrief() {
  ...
  const data = unwrap('briefs.latest', await supabase
    .from('briefs')
...
export async function getLatestStates() {
  if (!supabase) return [];
  const data = unwrap('briefs.latestStates', await supabase
    .from('briefs')
...
export async function getStateHistory() {
  if (!supabase) return {};
  const data = unwrap('briefs.stateHistory', await supabase
    .from('briefs')
...
export async function getClassificationHistory(limit = 24, asOf = null) {
  ...
  const data = unwrap('briefs.classificationHistory', await q
    .order('run_date', { ascending: false })
    .order('created_at', { ascending: false })
    .limit(limit));
...
export async function getCurrentCriteria() {
  if (!supabase) return {};
  const data = unwrap('gauge_criteria.current', await supabase
    .from('gauge_criteria')
...
export async function getCriteriaAsOf(runDate) {
  ...
  const data = unwrap('gauge_criteria.asOf', await supabase
    .from('gauge_criteria')
...
export async function getCriteriaRegister() {
  if (!supabase) return [];
  return unwrap('gauge_criteria.register', await supabase
    .from('gauge_criteria')
    .select('*')
    .order('effective_date', { ascending: false })
...
export async function getDeskConfig(keys = []) {
  ...
  const data = unwrap('desk_config', await q);
  const map = {};
...
export async function getAllBriefs(ascending = false) {
  if (!supabase) return [];
  return unwrap('briefs.all', await supabase
    .from('briefs')
...
export async function getBriefById(id) {
  if (!supabase) return null;
  const data = unwrap('briefs.byId', await supabase
    .from('briefs')
    .select('*, dashboard_states(*), brief_items(*)')
    .eq('published', true)
    .eq('id', id)
...
export async function getClaims() {
  if (!supabase) return [];
  const data = unwrap('claims', await supabase
    .from('claims')
...
export async function getWatchItems() {
  if (!supabase) return [];
  return unwrap('watch_items', await supabase
    .from('watch_items')
    .select('*')
    .order('created_at', { ascending: true })) ?? [];
}
