...
// Read reliability, in three layers. Added Sept 14, 2026 after a live failure.
//
// What happened: for about sixteen minutes the Supabase REST layer returned
// 502 and 504 across every table, with origin times of 5 to 35 seconds. The
// readers here destructured only `data` and dropped `error`, so a failed read
// was indistinguishable from an empty table. getClaims returned [], and the
// claim tracker told readers "No tracked claims yet" while 135 claims sat
// intact in the database. Nothing threw, so ISR cached that render.
//
// Layer 1, retry. Individual failures in that window were transient, seconds
// each. Two retries with exponential backoff and jitter ride straight through
// the common case, which is the actual fix: the reader never sees anything.
//
// Layer 2, don't retry what can't succeed. A malformed query, a missing
// column or an RLS refusal fails identically every time. Retrying those wastes
// a second and adds load to an instance that may already be struggling, so
// deterministic errors fail fast.
//
// Layer 3, throw when retries are exhausted. Next abandons the regeneration
// and keeps serving the last good page. That is a floor, not a strategy: it is
// there so a sustained outage degrades to slightly stale rather than to a
// confident falsehood about the desk's own record. Pages that care about
// freshness should surface their own "as of" stamp rather than rely on this.
...
function isRetryable(error) {
  ...
  // Postgres SQLSTATE (42501 permission denied, 42703 undefined column,
  // 22P02 invalid input) and PostgREST request errors (PGRST116 and friends)
  // are deterministic. The same query fails the same way on every attempt.
  if (/^\d{2}[0-9A-Z]{3}$/.test(code)) return false;
  if (code.startsWith('PGRST')) return false;
  // Everything else, gateway timeouts, connection resets, fetch failures,
  // is worth another go.
  return true;
}
...
// `build` must be a function returning a fresh query, not an awaited result:
// Supabase query builders are thenable and single use, so a retry has to
// construct a new one rather than await the same object twice.
async function read(what, build) {
  let lastError;
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    try {
      ...
      // A network-level throw (DNS, socket reset, abort) never reaches the
      // { data, error } shape, so normalise it into one.
    ...
    // Exponential backoff with jitter, so concurrent regenerations of
    // different routes do not retry in lockstep and re-saturate the pool
    // they are waiting on.
  ...
  throw new Error(
    `Supabase read failed (${what}) after ${MAX_ATTEMPTS} attempts: ${detail}`
  );
}

export async function getLatestBrief() {
  if (!supabase) return null;
  const data = await read('briefs.latest', () => supabase
    .from('briefs')
...
// Just enough of the latest brief to build a link preview. Added Sept 17, 2026.
//
// Deliberately not getLatestBrief(). generateMetadata runs as its own render
// pass, so reusing the full reader would pull every column of the brief plus
// its dashboard_states and brief_items joins a second time in order to read
// three fields, on a path that every social crawler hits. The gauge levels come
// along only so the embed accent can be keyed to the worst active gauge if that
// is ever wanted; nothing reads them yet.
export async function getLatestBriefMeta() {
  if (!supabase) return null;
  const data = await read('briefs.latestMeta', () => supabase
    .from('briefs')
    .select('id, run_date, headline, teaser_md, dashboard_states(level)')
    .eq('published', true)
    .order('run_date', { ascending: false })
    .order('created_at', { ascending: false })
    .limit(1));
...
export async function getLatestStates() {
  if (!supabase) return [];
  const data = await read('briefs.latestStates', () => supabase
    .from('briefs')
...
export async function getStateHistory() {
  if (!supabase) return {};
  const data = await read('briefs.stateHistory', () => supabase
    .from('briefs')
...
export async function getClassificationHistory(limit = 24, asOf = null) {
  ...
  const data = await read('briefs.classificationHistory', () => {
    let q = supabase
      .from('briefs')
...
export async function getCurrentCriteria() {
  if (!supabase) return {};
  const data = await read('gauge_criteria.current', () => supabase
    .from('gauge_criteria')
...
export async function getCriteriaAsOf(runDate) {
  ...
  const data = await read('gauge_criteria.asOf', () => supabase
    .from('gauge_criteria')
...
export async function getCriteriaRegister() {
  if (!supabase) return [];
  const data = await read('gauge_criteria.register', () => supabase
    .from('gauge_criteria')
    .select('*')
    .order('effective_date', { ascending: false })
...
export async function getDeskConfig(keys = []) {
  if (!supabase) return {};
  const data = await read('desk_config', () => {
...
export async function getAllBriefs(ascending = false) {
  if (!supabase) return [];
  const data = await read('briefs.all', () => supabase
    .from('briefs')
...
// Published briefs with the fields the RSS feed and the sitemap need, newest
// first. Added Sept 24, 2026. Reads only published rows, same as every public
// reader; published_at is the flip time and last_updated_at the last edit.
export async function getFeedBriefs(limit = 30) {
  if (!supabase) return [];
  const data = await read('briefs.feed', () => supabase
    .from('briefs')
    .select('id, run_date, brief_mode, headline, teaser_md, published_at, created_at, last_updated_at')
    .eq('published', true)
    .order('run_date', { ascending: false })
    .order('created_at', { ascending: false })
    .limit(limit));
...
export async function getBriefById(id) {
  if (!supabase) return null;
  const data = await read('briefs.byId', () => supabase
    .from('briefs')
    .select('*, dashboard_states(*), brief_items(*)')
    .eq('published', true)
    .eq('id', id)
...
export async function getClaims() {
  if (!supabase) return [];
  const data = await read('claims', () => supabase
    .from('claims')
...
export async function getWatchItems() {
  if (!supabase) return [];
  const data = await read('watch_items', () => supabase
    .from('watch_items')
    .select('*')
    .order('created_at', { ascending: true }));
  return data ?? [];
